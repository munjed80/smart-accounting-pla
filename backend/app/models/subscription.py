...
class PendingWebhookRetry(Base):
    """Persist webhook events that need asynchronous retry processing."""
    __tablename__ = "pending_webhook_retries"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    provider: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    resource_id: Mapped[str] = mapped_column(String(255), nullable=False, unique=True, index=True)
    resource_type: Mapped[str] = mapped_column(String(50), nullable=False)  # payment | subscription
