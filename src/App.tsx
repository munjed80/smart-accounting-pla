...
const AppContent = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'clients' | 'workqueue' | 'reviewqueue' | 'reminders' | 'acties' | 'bank' | 'crediteuren' | 'profitloss' | 'grootboek' | 'transactions' | 'upload' | 'settings' | 'support' | 'boekhouder' | 'zzp-customers' | 'zzp-invoices'>('dashboard')
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'zzp-customers':
        // ZZP-only page for customers
        return !isAccountant ? <ZZPCustomersPage /> : <SmartDashboard />
      case 'zzp-invoices':
        // ZZP-only page for invoices
