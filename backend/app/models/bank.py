...
class BankTransactionStatus(str, enum.Enum):
    ...
    MATCHED = "MATCHED"            # Matched to an invoice, expense, or transfer
    IGNORED = "IGNORED"            # Manually ignored (e.g., internal transfers)
    NEEDS_REVIEW = "NEEDS_REVIEW"  # Flagged for accountant review


class MatchedType(str, enum.Enum):
    """Type of entity the bank transaction is matched to."""
    INVOICE = "INVOICE"      # Matched to a customer invoice (open item)
    EXPENSE = "EXPENSE"      # Matched to a vendor expense/payable
    TRANSFER = "TRANSFER"    # Internal bank transfer
    MANUAL = "MANUAL"        # Manually created journal entry


class ReconciliationActionType(str, enum.Enum):
    """Types of actions that can be taken during reconciliation."""
    ACCEPT_MATCH = "ACCEPT_MATCH"      # Accept a suggested match
    IGNORE = "IGNORE"                  # Ignore this transaction
    CREATE_EXPENSE = "CREATE_EXPENSE"  # Create a new expense entry
    LINK_INVOICE = "LINK_INVOICE"      # Link to an existing invoice
...
class BankTransaction(Base):
    ...
    Each transaction has a raw_hash for idempotent imports:
    hash = SHA256(administration_id + booking_date + amount + description + reference)
    """
    ...
    raw_hash: Mapped[str] = mapped_column(String(64), nullable=False)  # SHA256 hash
    ...
    matched_type: Mapped[MatchedType] = mapped_column(
        SQLEnum(MatchedType), nullable=True
    )
...
class ReconciliationAction(Base):
    ...
    user = relationship("User", back_populates="reconciliation_actions")
